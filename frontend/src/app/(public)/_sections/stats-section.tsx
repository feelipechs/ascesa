        ...
        <SortableList items={optimisticItems} onReorder={handleReorder}>
            ...
            {optimisticItems.map((stat) => (
              <SortableItem key={stat.id} id={stat.id}>
                {({ attributes, listeners, isDragging }) => (
                  <Stat
                    className={`relative flex-col items-center text-center gap-1 py-4 border-0 bg-transparent shadow-none rounded-none p-0 grid-cols-1 ${isDragging ? 'opacity-50' : ''}`}
                  >
                    ...
                    <StatValue className="text-3xl md:text-4xl font-bold text-primary tracking-tight">
                      <NumberTicker
                        value={Number(
                    ...
                    <StatLabel className="text-xs uppercase tracking-wider font-medium">
                      {stat.label}
                    </StatLabel>
                  </Stat>
                )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-border">
            {optimisticItems.map((stat) => (
              <Stat
                key={stat.id}
                className="relative flex-col items-center text-center gap-1 py-4 border-0 bg-transparent shadow-none rounded-none p-0 grid-cols-1"
              >
                <StatValue className="text-3xl md:text-4xl font-bold text-primary tracking-tight">
                  <NumberTicker
                    value={Number(
