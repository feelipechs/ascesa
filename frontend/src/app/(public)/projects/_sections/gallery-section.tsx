...
export function GallerySection({ projectId, isAuthenticated }: GallerySectionProps) {
  ...
  const { data: images = [] } = useGalleryImages({ context: 'PROJECT', projectId })
  const { remove, isPending } = useGalleryImageMutations()

  if (images.length === 0 && !isAuthenticated) return null
