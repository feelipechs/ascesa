...
type TeamSectionWrapperProps = {
  ...
  areaId: string
  initialMembers?: TeamMemberWithAreas[]
}

export function TeamSectionWrapper({ isAuthenticated, areaId, initialMembers }: TeamSectionWrapperProps) {
  ...
  const linkedMembers = useMemo(
    () => {
      if (allMembers) {
        return allMembers.filter((m) => m.areas.some((a) => a.area.id === areaId))
      }
      return initialMembers ?? []
    },
    [allMembers, areaId, initialMembers]
  )
